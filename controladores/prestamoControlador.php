<?php
    ...
    class prestamoControlador extends prestamoModelo{
		...
		public function agregar_item_prestamo_controlador(){
			...
			// CORRECCIÓN: Cambiar la lógica de validación (agregar el operador !)
			...
			if(!mainModel::verificar_datos("[0-9]{1,7}",$tiempo)){
				$alerta=[
					...
					"Texto"=>"El tiempo debe ser un número válido.",
					"Tipo"=>"error"
				];
				echo json_encode($alerta);
				exit();
			...
			if(!mainModel::verificar_datos("[0-9.]{1,15}",$costo)){
				$alerta=[
					...
					"Texto"=>"El costo debe ser un número decimal válido.",
					"Tipo"=>"error"
				];
				echo json_encode($alerta);
				exit();
			...
			if(intval($tiempo) < 1){
				$alerta=[
					"Alerta"=>"simple",
					"Titulo"=>"Error en el tiempo",
					"Texto"=>"El tiempo debe ser mayor a 0.",
					"Tipo"=>"error"
				];
				echo json_encode($alerta);
				exit();
			...
			if(floatval($costo) < 0){
				$alerta=[
					"Alerta"=>"simple",
					"Titulo"=>"Error en el costo",
					"Texto"=>"El costo no puede ser negativo.",
					"Tipo"=>"error"
				];
				echo json_encode($alerta);
				exit();
			...
			if(empty($_SESSION['datos_item'][$id])){
				...
				$tiempo_num = intval($tiempo);
				$costo_num = floatval($costo);
				
				// CALCULAR EL SUBTOTAL: cantidad × tiempo × costo_por_tiempo
				$subtotal = $cantidad_num * $tiempo_num * $costo_num;
				
				// Formatear valores
				$costo_formateado = number_format($costo_num, 2, '.', '');
				$subtotal_formateado = number_format($subtotal, 2, '.', '');
				
				// GENERAR UN ID ÚNICO PARA EL CARRITO
				$carrito_id = uniqid();

				// GUARDAR EN SESIÓN con las claves correctas que espera la vista
				$_SESSION['datos_item'][$id]=[
					...
					"TIEMPO"=>$tiempo_num,
					"COSTO_TIEMPO"=>$costo_num,  // Valor numérico para cálculos
					"SUBTOTAL"=>$subtotal        // Valor numérico para cálculos
				];
				
				$alerta=[
					"Alerta"=>"recargar",
					"Titulo"=>"¡Ítem agregado!",
					"Texto"=>"El item se agregó correctamente. Cantidad: $cantidad_num, Tiempo: $tiempo_num, Costo: $$costo_formateado, Subtotal: $$subtotal_formateado",
					"Tipo"=>"success"
				];
				echo json_encode($alerta);
				exit();
